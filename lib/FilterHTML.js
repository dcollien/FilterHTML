var FilterHTML = (function() {
   ...
   var HTMLFilter = function(spec, allowed_schemes, text_filter, remove_scripts, remove_styles) {
      this.html = '';
      this.filtered_html = '';
      this.spec = spec;
      ...
      if (remove_scripts !== null) {
         this.remove_scripts = remove_scripts;
      } else {
         this.remove_scripts = false;
      }

      if (remove_styles !== null) {
         this.remove_styles = remove_styles;
      } else {
         this.remove_styles = false;
      }
   };
      ...
      while (this.next()) {
         if (this.curr_char === '<') {
            ...
            if (this.state === 'script-data' && this.remove_scripts) {
               // pass
            } else if (this.state === 'style-data' && this.remove_styles) {
               // pass
            } else {
               text_chars += this.escape_data(this.curr_char);
            }
   ...
   HTMLFilter.prototype.filter_closing_tag = function() {
      ...
      if (tag_name === 'script' && this.state === 'script-data') {
         this.state = 'data';
      } else if (tag_name === 'style' && this.state === 'style-data') {
         this.state = 'data';
      }
